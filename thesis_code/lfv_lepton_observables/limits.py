...
def radiative_decay_limit(m, process, idx, g = None, th = [[0]*3]*3, d = [[0]*3]*3, ph = [[0]*3]*3,  mode = None, ALP = False, Lam = 1000, confidence = 0.9):
    """
    Calculate the coupling limit from radiative decay experiments.
    
    This function computes the upper limit on coupling strength from
    radiative decay experiments like μ → eγ, τ → eγ, τ → μγ.
    ...
    confidence : float, optional
        Confidence level for the limit (default 0.9 for 90%)
    
    Returns
    -------
    float
        Upper limit on coupling strength
    """
...
def trilepton_decay_limit(m, process, idx, g = None, th = [[0]*3]*3, d = [[0]*3]*3, ph = [[0]*3]*3,  mode = None, ALP = False, Lam = 1000, confidence = 0.9):
    """
    Calculate the coupling limit from trilepton decay experiments.
    
    This function computes the upper limit on coupling strength from
    trilepton decay experiments like μ → 3e, τ → 3e, τ → 3μ.
    ...
    m : float
        Scalar/ALP mass
    process : tuple
        Decay process (i, j, k, l) for li → lj lk l̅l
    idx : tuple
        Indices (i, j, k, l) specifying which couplings to constrain
    g : array-like, optional
        Coupling matrix (if None, uses unit couplings for specified indices)
...
def magnetic_dipole_moment_limit(m, i, idx, g = None, th = [[0]*3]*3, d = [[0]*3]*3, ph = [[0]*3]*3,  mode = None, ALP = False, Lam = 1000, confidence = 0.9):
    """
    Calculate the coupling limit from magnetic dipole moment measurements.
    
    This function computes the upper limit on coupling strength from
    precision measurements of lepton magnetic dipole moments (g-2).
...
def electric_dipole_moment_limit(m, i, idx, g = None, th = [[0]*3]*3, d = [[0]*3]*3, ph = [[0]*3]*3,  mode = 'max CPV', ALP = False, Lam = 1000, confidence = 0.9):
    """
    Calculate the coupling limit from electric dipole moment measurements.
    
    This function computes the upper limit on coupling strength from
    precision measurements of lepton electric dipole moments.
    ...
    ph : array-like, optional
        C or CP violating phases for couplings
    mode : str, optional
        Coupling mode: 'max CPV' for maximal CP violation (default)
    ALP : bool, optional
        Whether to include ALP photon contributions
    Lam : float, optional
        ALP EFT scale (used only if ALP=True)
...
def g_2_explanation(m, which_anomaly, idx, g = None, th = [[0]*3]*3, d = [[0]*3]*3, ph = [[0]*3]*3,  mode = None, ALP = False, Lam = 1000, nsig = 2):
    """
    Calculate coupling values needed to explain g-2 anomalies.
    
    This function computes the range of coupling values that could
    explain the observed g-2 anomalies within the specified confidence.
    ...
    -------
    tuple
        (lower_bound, upper_bound) coupling values that explain the anomaly
    """

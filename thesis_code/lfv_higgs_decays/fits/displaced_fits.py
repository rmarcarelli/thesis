...
"""
Fitting functions for displaced vertex analyses.

This module provides fitting functions for ATLAS and MATHUSLA displaced
vertex searches. It includes piecewise polynomial fits and decay probability
calculations for long-lived particle searches.
"""
...
def piecewise_fit(X,l00,l01,l02,l03,l10,l11,l12,l13,l20,l21,l22,l23,
    ...
    """
    Piecewise polynomial fit for ATLAS displaced vertex analysis.
    
    This function implements a complex piecewise polynomial fit with
    continuity constraints at boundaries. It uses separate polynomials
    for left, middle, and right regions with smooth transitions.
    
    Parameters
    ----------
    X : tuple
        (ma, ta) where ma is ALP mass and ta is decay length
    l00-l23 : float
        Coefficients for left region polynomials
    m40-m83 : float
        Coefficients for middle region polynomials  
    r00-r23 : float
        Coefficients for right region polynomials
        
    Returns
    -------
    float or array-like
        Fitted branching fraction values
    """
    ma, ta = X
    L = np.log10(0.2)
    R = np.log10(2)
...
def ATLAS_fit(ma, ta):
    """
    ATLAS displaced vertex fit function.
    
    This function implements the fitted branching fraction limits for
    ATLAS displaced vertex searches. It uses piecewise polynomial fits
    with slope corrections to match the expected behavior.
    ...
    -------
    float or array-like
        Fitted branching fraction limits
    """
...
def Pdecay(x, L1, L2):
    """
    Calculate decay probability between two distances.
    
    Parameters
    ----------
    x : float or array-like
        Decay length parameter
    L1 : float
        Inner distance
    L2 : float
        Outer distance
    ...
    -------
    float or array-like
        Decay probability P(exp(-L1/x) - exp(-L2/x))
    """
    return np.exp(-L1/x) - np.exp(-L2/x)

def Pdecay_MATH(ta, ma, L1, L2):
    """
    Calculate MATHUSLA decay probability.
    ...
    ma : float or array-like
        ALP mass in GeV
    L1 : float
        Inner detector distance in meters
    L2 : float
        Outer detector distance in meters
        
    Returns
    -------
    float or array-like
        Decay probability in MATHUSLA detector
    """
    b = mH/(2*ma)
    x = b*ta
...
def Nobs(ta, ma, L1, L2):
    """
    Calculate expected number of observed events in MATHUSLA.
...
def MATH_approx(ta, ma, L1, L2):
    """
    Approximate MATHUSLA limit (underestimates).
    ...
    -------
    float or array-like
        Approximate branching fraction limit
    """
...
def piecewise_fit_MATH(X, m10, m11, m12, m13, m20, m21, m22, m23, m30, m31, m32, m33,
                       m40, m41, m42, m43, m50, m51, m52, m53, m60, m61, m62, m63):
    """
    Piecewise polynomial fit for MATHUSLA displaced vertex analysis.
    
    This function implements a piecewise polynomial fit for MATHUSLA
    with separate handling for left and right regions.
    ...
    X : tuple
        (ma, ta) where ma is ALP mass and ta is decay length
    m10-m63 : float
        Polynomial coefficients for different regions
...
def MATH_fit(ma, ta):
    """
    MATHUSLA displaced vertex fit function.
    
    This function implements the fitted branching fraction limits for
    MATHUSLA displaced vertex searches. It combines piecewise polynomial
    fits with approximate calculations for different regions.

...
from dipole_form_factors.formulae.special_functions import B, C0
...
def f(ui, uj, uk):    
    #Unfortunately, mpmath does not work with arrays
    if hasattr(ui, '__iter__'):
        return np.array([f(ui_, uj_, uk_) for ui_,uj_,uk_ in zip(ui,uj,uk)])
...
def g_plus(ui, uj):
    if hasattr(ui, '__iter__'):
        return np.array([g_plus(ui_, uj_) for ui_,uj_ in zip(ui,uj)])
...
def g_minus(ui, uj):
    if hasattr(ui, '__iter__'):
        return np.array([g_minus(ui_, uj_) for ui_,uj_ in zip(ui,uj)])
    ui = mp.mpc(ui)
    uj = mp.mpc(uj)

...
We begin by matching the UV theory onto the low energy ALP-lepton interaction by computing the effective ALP-lepton coupling $C_{\ell \ell'}$.  This requires a detailed matching calculation of dark pion decay into lepton pairs in the UV-complete model, $\Pi \rightarrow \ell_a^- \ell_b^+$.  In general, because confinement is a strong-coupling process, we would need a non-perturbative approach such as lattice calculation to predict the properties of the infrared theory.  However, since the $\Pi$ is a pGNB associated with chiral symmetry breaking, we can match on to chiral perturbation theory in the infrared. 
...
\begin{align}
    i{\cal M} &= \lambda_a'\lambda_b'^* C \bar{v}(p_j)\gamma^5 \left[G^{ij-}_{ab} \gamma_\nu {\cal I}_1^{\mu\nu} + G^{ij+}_{ab}{\cal I}_2^{\mu} \right]u(p_i)\bar{u}(k_a)\gamma_\mu P_L v(k_b)
\end{align}
where $C$ is a dark color factor, $p_i$ and $p_j$ are the momenta of the dark quarks $\psi_i$ and $\psi_j$, $k_a$ and $k_b$ are the momenta of the leptons $\ell_a$ and $\ell_b$, $G_{ab}^{ij\pm} = g_{aS}^{ij}g_{bPS}^{ij*} \pm g_{aPS}^{ij}g_{bS}^{ij*}$, and ${\cal I}_1$ and ${\cal I}_2$ are integral expressions given by
\begin{align}
...
\begin{align}
    & {\cal I}_{1}^{\mu\nu} = \frac{ig^{\mu\nu}}{64\pi^2 M_S^2}g_1(M_F^2/M_S^2),
    & {\cal I}_{2}^{\mu} = \frac{i}{64\pi^2 M_S^2}\frac{m(p_i^\mu - k_a^\mu)}{M_S^2}g_2(M_F^2/M_S^2), &
\end{align}
where
\begin{align}
...
\end{align}
Taking $M_F \approx M_S$, we note that $\lim_{x\rightarrow 1}g_1(x) = -\lim_{x\rightarrow 1}g_2(x) = 1/2$. In addition, $I_2 \ll I_1$ due to a suppression of $m_k/M_S$ (and a potential additional suppression by $(p_i^\mu - k_a^\mu)/M_S$ as well depending on the energy scale of the interaction). Finally, we note that the color factor can be found from the contraction $\epsilon_{\alpha\beta\gamma}\delta_{\rho}^{\gamma}\epsilon^{\rho\sigma\tau}\delta_{\tau}^{\beta}=\epsilon_{\alpha\beta\gamma}\epsilon^{\gamma\sigma\beta} = 2\delta_\alpha^\sigma$, yielding $C = 2$. Hence, the amplitude at low energy scales ($\ll M_S$) can be written
\begin{align}
    i{\cal M} &\approx \frac{\lambda_a'\lambda_b'^* G_{ab}^{ij-}}{64\pi^2 M_S^2}[\bar{v}(p_j)\gamma^5 \gamma^\mu u(p_i)][ \bar{u}(k_a) \gamma_\mu P_L v(k_b)],
\end{align}
corresponding to an effective interaction 
\begin{align}
    {\cal L}_{\psi\psi\ell\ell} &= \sum_{a,b}\frac{\lambda_a'\lambda_b'^*}{64\pi^2 M_S^2} \sum_{i,j}\left[iG_{ab}^{ij-}\bar{\psi}_j \gamma^\mu \gamma^5 \psi_i\right]_{{\Bbb Z}_2 = +} \bar{\ell}_a \gamma_\mu P_L \ell_b\label{eq:dark_quark_eff_int}
\end{align}
...
\begin{align}
    {\cal L}_{\psi\psi\ell\ell} &= \sum_{a,b}\frac{\lambda_a'\lambda_b'^*}{64\pi^2 M_S^2} \sum_{i,j}{
...
\begin{align}
    \frac{C_{ab}^x}{\Lambda} &= \frac{\lambda_a'\lambda_b'^*}{64\pi^2}\frac{F_{\Pi_x}}{M_S}\frac{iG_{ab}^{x-}}{M_S},
\end{align}
or in terms of the original parameters of the UV Lagrangian,
\begin{equation}
\frac{C_{ab}^x}{\Lambda} = \frac{\lambda_a^\prime\lambda_b^{\prime*}}{128\pi^2M_S}\sum_{i,j}i\left[g_{aS}^{ij}g_{bPS}^{ij*} - g_{aPS}^{ij}g_{bS}^{ij*}\right]\frac{F_{\Pi_x}}{M_S}\lambda_{ij}^x.
\end{equation}
...
\begin{align}
    \frac{C_{\ell\ell'}}{\Lambda} \sim \frac{\lambda'^2 g^2}{128\pi^2 M_S}\frac{F_\Pi}{M_S}.
\end{align}
Adopting the numerical values for the benchmark UV model in Ref.~\cite{Davoudiasl:2017zws} of $\lambda' \approx 0.1$,  $g \approx 0.3$, $F_\Pi \sim 80$ GeV, and $M_S \sim 1$ TeV we find the numerical value
%
\begin{equation}
\frac{|C_{\ell \ell'}|}{\Lambda} \approx 5 \times 10^{-8}\ {\rm TeV}^{-1},
\end{equation}
which is far too small to be probed by the LFV experiments discussed in Chapter \ref{lfv}.


\section{Higgs couplings}

Now we match the UV Lagrangian onto the Higgs-ALP interaction by computing the effective couplings $C_{ah}$ and $C_{ah}'$ in the low-energy ALP Lagrangian. In Ref.~\cite{Davoudiasl:2017zws}, the Higgs-dark-quark effective interaction from the UV Lagrangian is found to be
\begin{align}
    {\cal L}_{H\psi} &= \frac{m_i}{v_H^2} H^\dagger H \bar{\psi}_i\psi_i\label{eq:dark_quark_mass}
\end{align}
with $m_i \approx \lambda_i^2 v_H^2 / 2M_F \sim 0.03~{\rm GeV}$ in the proposed benchmark model. Then, the Higgs-dark-pion interaction can be found analogously to the Higgs-pion interaction in chiral perturbation theory in QCD. For pions in the Standard Model, both couplings arise; however, the derivative $C_{ah}$ coupling arises due to matching from the coupling of the Higgs to the gluonic $G \tilde{G}$ operator, which in turn is induced by integrating the heavy quarks $c,b,t$ out of the theory.  In the present model, there are no such heavy dark fermions, so that the Higgs coupling to axions is entirely due to matching on to the same operator interaction (\ref{eq:dark_quark_mass}) responsible for generation of dark quark mass.  This leads to generation of only the $C_{ah}'$ operator when matching at leading order.  We have
%
\begin{equation}
\mathcal{L}_{\Pi h} = \sum_{\Pi} \frac{1}{2v_H} M_\Pi^2 \Pi^2 h =  \sum_{\Pi} \frac{C'_{\Pi h}}{\Lambda^2} v_H M_\Pi^2 \Pi^2 h
\end{equation}
%
where the sum is over the four light meson states, and we are defining the effective coupling
%
\begin{equation}
\frac{C'_{\Pi h}}{\Lambda^2} \equiv \frac{1}{2v_H^2}.
\end{equation}
%
...
\frac{C_{ah}'}{\Lambda^2} = \frac{1}{v_H^2} \approx 16\ {\rm TeV}^{-2}.
\end{equation}
%
Whereas the lepton coupling $C_{\ell\ell'}/\Lambda$ from the benchmark model is far too small to be probed by current experiments, the Higgs coupling is comparatively much larger. Given the flavor-violating nature of the lepton interaction and the relative strength of the Higgs interaction, this model may be probed in searches for LFV Higgs decays in the near future, as discussed in the text. 

It is interesting to note that the result Eq.~(\ref{eq:Cahp_UV}) is completely independent of the parameters of the UV model.\footnote{up to the number of degenerate ALP states $N_\Pi$: to recast in general, we would constrain the combination $C_{ah}' / \Lambda^2 = 1/({\sqrt{N_\Pi} v_H^2})$.}  This indicates that the same result would hold in any composite axion model satisfying the conditions that all fermion mass is generated by Higgs Yukawa couplings, and that there are no additional heavy fermions in the composite sector that would give rise to a $C_{ah}$ coupling.

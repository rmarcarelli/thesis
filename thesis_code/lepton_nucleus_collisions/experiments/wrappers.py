"""
Experiment wrapper utilities for lepton-nucleus collision calculations.

This module provides decorators for unit conversion and cached evaluation
...
def convert():
    """
    Decorator for unit conversion of cross sections.
    
    This decorator converts cross sections from natural units (GeV^-2) to
    fb or pb, and optionally multiplies by a coupling g^2.
    
    Returns
    -------
    function
        Decorated function with unit conversion capability
        
    Notes
    -----
    Only applies to (differential) cross sections.
    Supports units: 'GeV' (natural), 'pb', 'fb'.
    """
    def decorator(func):
...
def cached_evaluation(key):
    """
    Decorator for cached evaluation of cross section calculations.
    
    This wrapper checks whether data is cached, and if so, pulls it from
    the experiment's H5 file. Can also interpolate between cached mass values.
    
    Parameters
    ----------
    key : str
        Cache key for the data ('crossx', 'dcrossx_dx', 'dcrossx_dy', 'dcrossx_dx_dy')
    ...
    Notes
    -----
    Supports mass interpolation when interpolate_mass=True.
    Falls back to direct computation when cache is unavailable.
    """
    def decorator(func):
